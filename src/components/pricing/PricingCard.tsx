...
export function PricingCard({
  ...
  return (
    <div
      ...
      } ${isEditMode ? 'w-72' : 'cursor-pointer hover:border-gray-300 hover:shadow-md'}`}
      onClick={!isEditMode ? onCardClick : undefined}
    >
      {/* Popular Badge */}
      {config.highlighted && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-10">
          <span className="bg-[#253ff6] text-white text-xs font-semibold px-4 py-1.5 rounded-[0.2rem] flex items-center gap-1.5 shadow-lg shadow-[#253ff6]/25">
      ...
      {isComingSoon && !config.highlighted && (
        <div className="absolute -top-3 right-4 z-10">
          <span className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[11px] font-semibold px-3 py-1 rounded-[0.2rem] shadow-sm">
            Coming Soon
          </span>
        </div>
      )}

      <div className={`p-6 ${config.highlighted ? 'pt-8' : ''}`}>
        {/* Tier Header */}
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-lg font-semibold text-gray-900">{tier.name}</h3>
            {tier.id === 'enterprise' && <Crown size={18} weight="fill" className="text-amber-500" />}
          </div>
        ...
        <div className="mb-6">
          {isCustom ? (
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-sm font-medium text-gray-400">MYR</span>
          ...
          {billingCycle === 'annual' && config.monthlyPrice > 0 && !isCustom && (
            <p className="text-sm text-emerald-600 mt-2 font-medium">
              Save MYR {(config.monthlyPrice * 12 - config.annualPrice).toLocaleString()}/year
            </p>
          )}
        </div>
        ...
        <button
          ...
          className={`w-full py-3 px-4 rounded-[0.2rem] font-medium text-sm transition-all duration-200 mb-6 ${
            config.ctaStyle === 'primary'
              ? 'bg-[#253ff6] text-white hover:bg-[#1e35d4] shadow-md shadow-[#253ff6]/20'
        ...
        <div>
          ...
          <div className="space-y-3">
            {config.highlightedFeatures.slice(0, config.maxVisibleFeatures).map((featureId) => {
              ...
              return (
                <div key={featureId} className="flex items-start gap-3">
                  <div className="w-5 h-5 rounded-[0.2rem] bg-emerald-50 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <Check size={12} weight="bold" className="text-emerald-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="text-sm text-gray-700">{getFeatureName(featureId)}</span>
            ...
            {config.highlightedFeatures.length === 0 && (
              <p className="text-sm text-gray-400 italic">Click to add features</p>
            )}
